...
All packages in this monorepo depend on the [shared package](packages/shared) (`@callstack/licenses-api`). This package needs to be rebuilt whenever you make changes to it since other packages consume the exported, built JS code with emitted typings. You can do this by running the following command from the root directory, which will start `tsc` in watch mode:

```sh
yarn workspace @callstack/licenses-api dev
```
